...
filtered_reports = list(filter(lambda r: "http" in r["text"], reports))

total_request_level = list(map(
    lambda r: dict(r, text=r["text"][:r["text"].find("http")] + "[ДАННЫЕ УДАЛЕНЫ]"), 
    filtered_reports
))

print(*total_request_level, sep='\n')